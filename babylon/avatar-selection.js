...
export class AvatarSelection extends World {
  ...
  async createLights() {
    ...
    this.hemisphere = new BABYLON.HemisphericLight(
      "light1",
      new BABYLON.Vector3(1, 1, 0),
      this.scene
    );
    var point = new BABYLON.PointLight(
      "light2",
      new BABYLON.Vector3(1, 3, -3),
      this.scene
    );
    return point;
  }
  ...
  trackXrDevices() {
    if (
      ...
      if (this.vrHelper.leftController) {
        if (this.mirror) {
          var leftPos = this.calcControllerPos(
            this.character.body.leftArm,
            this.vrHelper.leftController
          );
          this.character.reachFor(this.character.body.leftArm, leftPos);
        } else {
          var leftPos = this.calcControllerPos(
            this.character.body.rightArm,
            this.vrHelper.leftController
          );
      ...
      if (this.vrHelper.rightController) {
        if (this.mirror) {
          var rightPos = this.calcControllerPos(
            this.character.body.rightArm,
            this.vrHelper.rightController
          );
  ...
  calcControllerPos(arm, xrController) {
    ...
    var pos = xrController.grip.absolutePosition.subtract(
  ...
  calcCameraTarget() {
    ...
    var target = new BABYLON.Vector3(
      0,
      this.vrHelper.camera().realWorldHeight,
      1
    );
    target.rotateByQuaternionAroundPointToRef(
      cameraQuat,
      this.character.headPos(),
      target
    );
    if (this.mirror) {
      target.z = -target.z;
  ...
  createSelection(selectionCallback) {
    ...
    VRSPACEUI.listMatchingFiles(this.characterDir(), (folders) => {
      ...
      var buttons = new Buttons(
        ...
        "Avatars",
        folders,
        (dir) => this.createAvatarSelection(dir),
        "name"
      );
      buttons.setHeight(0.5);
      buttons.group.position = new BABYLON.Vector3(0.5, 2.2, -0.5);
      buttons.select(0);
    ...
    if (folder.url) {
      VRSPACEUI.listCharacters(folder.url(), (avatars) => {
        var buttons = new Buttons(
          ...
          folder.name,
          avatars,
          (dir) => this.loadCharacter(dir),
          "name"
        );
        buttons.setHeight(0.1 * Math.min(20, avatars.length));
        buttons.group.position = new BABYLON.Vector3(1.3, 2.2, -0.5);
  ...
  animationButtons(avatar) {
    ...
    this.animationSelection = new Buttons(
      this.scene,
      "Animations",
      names,
      (name) => this.startAnimation(name)
    );
    this.animationSelection.turnOff = true;
    this.animationSelection.setHeight(Math.min(2, names.length / 10));
    this.animationSelection.group.position = new BABYLON.Vector3(-2, 2.2, -0.5);
  }
  ...
  addCharacterButtons() {
    ...
    resizeButton.node.scaling = new BABYLON.Vector3(0.2, 0.2, 0.2);
    ...
    mirrorButton.onPointerDownObservable.add(() => {
      ...
      if (mirrorButton.text == "Mirroring") {
        ...
        this.mirror = false;
        this.character.rootMesh.rotationQuaternion =
          BABYLON.Quaternion.RotationAxis(BABYLON.Axis.Y, 0);
      } else {
        mirrorButton.text = "Mirroring";
        this.mirror = true;
        this.character.rootMesh.rotationQuaternion =
  ...
  showActiveUsers() {
    fetch("/worlds/users").then((response) =>
      response.json().then((worldStats) => {
        if (worldStats) {
          worldStats.forEach((stat) => {
            //console.log(stat);
            if (this.portals[stat.worldName]) {
              if (stat.activeUsers > 0) {
                this.portals[stat.worldName].setTitle(
                  "Users: " + stat.activeUsers + "/" + stat.totalUsers
                );
              } else {
  ...
  async enterWorld(
    ...
    worldScript = "world.js"
  ) {
    console.log(
      "Entering world " + worldUrl + "/" + worldScript + " as " + avatarUrl
    );
    ...
    import(worldUrl + "/" + worldScript).then((world) => {
      var afterLoad = (world) => {
        ...
        this.worldManager.mediaStreams = new OpenViduStreams(
          this.scene,
          "videos"
        );
        var myProperties = {
        ...
        this.worldManager
          ...
          .then((welcome) => {
            ...
            if (this.character) {
              ...
              this.worldManager.addMyChangeListener((changes) =>
                controller.processChanges(changes)
              );
            }
            if (this.afterEnter) {
      ...
      world.WORLD.init(
        ...
        if (this.inXR) {
          ...
          this.vrHelper
            .camera()
            .setTransformationFromNonVRCamera(world.WORLD.camera);
